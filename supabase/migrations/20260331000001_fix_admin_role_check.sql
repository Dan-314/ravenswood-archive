-- Fix admin role check across all RLS policies.
-- The JWT top-level 'role' field is always 'authenticated' for logged-in users.
-- The custom admin role lives at user_metadata.role, accessed via:
--   (auth.jwt() -> 'user_metadata' ->> 'role') = 'admin'
--
-- Prior policies used (auth.jwt() ->> 'role') = 'admin' which never matched,
-- meaning admin operations only worked when the admin was also the resource owner.
