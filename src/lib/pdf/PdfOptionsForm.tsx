...
import { useBackgrounds } from "@/lib/pdf/useBackgrounds";
...
export function PdfOptionsForm({ options, onUpdate }: PdfOptionsFormProps) {
  ...
  useEffect(() => {
    if (backgrounds.length > 0 && !options.background) {
      const bg = backgrounds[Math.floor(Math.random() * backgrounds.length)];
      onUpdate("background", bg.value);
      if (bg.attribution) onUpdate("backgroundAttribution", bg.attribution);
    }
  }, [backgrounds, options.background, onUpdate]);

  const colorValue = Array.isArray(options.color) ? options.color[0] : options.color;
  const [colorText, setColorText] = useState(colorValue);

  // Sync text input when color changes externally (e.g. random button, color picker)
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <Select value={options.background} onValueChange={(v) => {
          if (!v) return;
          update("background", v);
          update("backgroundAttribution", backgrounds.find((b) => b.value === v)?.attribution);
        }}>
          <SelectTrigger className="w-full">
            <SelectValue>
              {backgrounds.find((b) => b.value === options.background)?.label ?? options.background}
            </SelectValue>
          </SelectTrigger>
          <SelectContent>
        ...
        <div className="flex gap-2 items-center">
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              if (isValidHex(e.target.value)) update("color", e.target.value);
            }}
        ...
        <div className="flex flex-col gap-3">
          ...
          {options.overleaf && (
            <div className="ml-6 flex flex-col gap-2">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <Checkbox checked={options.displayTravellers} onCheckedChange={(checked) => update("displayTravellers", !!checked)} />
                <span className="text-sm">Travellers on back page (if included)</span>
              </label>
            </div>
          )}
